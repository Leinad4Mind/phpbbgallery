		...
		if (!empty($missing_image_ids))
		{
			...
			while ($row = $this->phpbb_db->sql_fetchrow($result))
			{
				$this->images[(int) $row['image_id']] = [
					...
					'image_delete_previous_status' => $row['image_delete_previous_status'] ?? \phpbbgallery\core\block::STATUS_APPROVED,
					'image_delete_request_user_id' => $row['image_delete_request_user_id'] ?? 0,
					'image_delete_request_time' => $row['image_delete_request_time'] ?? 0,
