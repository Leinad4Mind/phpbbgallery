(function () {
	...
	function findOutput(textarea) {
		...
		while (container && container !== document) {
			var output = container.querySelector('[data-gallery-comment-counter-output]');
			if (output) {
				return output;
			}
	...
	function update(textarea, output) {
		var maximum = parseInt(textarea.getAttribute('data-comment-max-length'), 10);
	...
	function initialize(root) {
		var textareas = root.querySelectorAll('[data-gallery-comment-counter]');
		Array.prototype.forEach.call(textareas, function (textarea) {
			if (textarea.getAttribute('data-gallery-comment-counter-ready') === '1') {
				return;
			}
			...
			textarea.setAttribute('data-gallery-comment-counter-ready', '1');
