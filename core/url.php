...
class url
{
	...
	*/
	 ...
	 * @param string             $path
	 * @param string             $file
	 * @param string|array|false $params
	 * @param bool $is_amp
	 * @return string
	 */
	...
	*
	* @param string $path        Ugly path, e.g. "../community/../gallery/"
	* @param bool   $is_full_url Whether the path is a full URL
	...
	*/
	public static function beautiful_path(string $path, bool $is_full_url = false): string
	{
		...
		{
			if ($bits[$i] == '..' && isset($bits[$i - 1]) && $bits[$i - 1] !== '' && $bits[$i - 1][0] != '.')
			{
	...
	*/
	...
	{
		$route = $this->strip_share_context(html_entity_decode($route, ENT_QUOTES | ENT_HTML5, 'UTF-8'));
	...
	/**
	 * Remove request-specific session and style values from a shareable route.
	 *
	 * @param string $route Route which may contain query parameters
	 * @return string
	 */
	private function strip_share_context(string $route): string
	{
		$fragment = '';
		...
		$route = (string) preg_replace('#([?&])(?:sid|style)=[^&]*#i', '$1', $route);
