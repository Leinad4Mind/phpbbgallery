...
- Added a read-only **View Gallery permissions** page under the native ACP Permission Masks category, showing every effective boolean and numeric permission for a selected user and public or personal album scope; per-permission tracing shows each contributing group and direct-user setting, the running Yes/No/Never result and the final value.
